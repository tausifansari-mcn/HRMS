...
| Phase | Module | Task | Current State | Planned Change | Files Affected | DB Impact | API Impact | Frontend Impact | Roles Impacted | Test Required | Risk | Status | Approval Required |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 0 | Access Control | Design MySQL-authoritative backend RBAC transition | No approved implementation exists. Design must verify actual role/page/scope tables, reconciliation rules, mismatch reporting and negative API test requirements. No code implementation approved yet. New `/api/access/roles` endpoint, new `access.routes.ts`, new `access.service.ts`, and any Supabase `user_roles` mirror writes are not approved for implementation until design is complete and reviewed. | Produce design document covering: MySQL-authoritative role source, Supabase UI visibility mirror strategy, reconciliation rules, mismatch reporting, and negative API test plan | Design document only — no code files | None until implementation approved | None until implementation approved | None until implementation approved | Admin only | Design review only at this stage | Medium — touches auth layer | ⬜ PENDING design | YES before any implementation |

### 0.5 — Portal Security Fixes

| Phase | Module | Task | Current State | Planned Change | Files Affected | DB Impact | API Impact | Frontend Impact | Roles Impacted | Test Required | Risk | Status | Approval Required |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 0 | Portal | Write `portal_access_log` on every client request | Candidate security fix. Table defined, never written. The applied SQL scripts add schema and reference master records only; any post-execution table-content reconciliation requires separately authorised read-only verification. | Add INSERT to `portal.controller.ts` on authenticated client routes — design and implementation package requires approval | `backend/src/modules/portal/portal.controller.ts` | `portal_access_log` INSERT per request | No change to response shape | None | Client users | Login via OTP; verify row in `portal_access_log` | Low | ⬜ PENDING | YES before implementation |
| 0 | Portal | Env-gate demo bypass | Candidate security fix. `demo@mascallnet.com` issues JWT unconditionally — design and implementation package requires approval | Gate behind controlled environment configuration | `backend/src/modules/portal/portal.auth.service.ts`, `backend/.env.example` | None | OTP endpoint behaviour unchanged unless bypass gated | None | Client portal | Test with bypass unset — demo email must follow OTP flow | Low | ⬜ PENDING | YES before implementation |
...
| Phase | Module | Task | Current State | Planned Change | Files Affected | DB Impact | API Impact | Frontend Impact | Roles Impacted | Test Required | Risk | Status | Approval Required |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Access Control | Implement MySQL-authoritative RBAC (promoted from Phase 0 design) | Phase 0 design output required first. Implementation covers: `GET/POST/DELETE /api/access/roles`, role reconciliation report endpoint, and negative API access tests | Implement per approved design from Phase 0 | Per approved design | Per approved design | Per approved design | Per approved design | Admin only | Per approved design test plan | Medium — touches auth layer | 🔵 DEFERRED (awaiting Phase 0 design approval) | YES |
...
| Phase | Module | Task | Current State | Planned Change | Files Affected | DB Impact | API Impact | Frontend Impact | Roles Impacted | Test Required | Risk | Status | Approval Required |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2 | Assets | MySQL schema for assets | Supabase only | New SQL file `015_assets.sql` | New SQL file | Additive | None yet | None yet | Admin, HR | Table creation on local MySQL | Low | 🔵 DEFERRED | YES before implementation |
| 2 | Assets | Backend `/api/assets` module | No backend route | New module mirroring `useAssets.ts` structure | New module | Reads/writes new MySQL table | New `/api/assets` endpoints | `useAssets.ts` feature-flagged to backend | Admin, HR | CRUD operations on assets | Medium | 🔵 DEFERRED | YES before implementation |
| 2 | Assets | Migrate Supabase assets → MySQL | Data in Supabase | One-click migration via migration console | `migration.service.ts` | Writes MySQL `assets` | Migration endpoint | None | Admin | Counts match pre/post | Medium | 🔵 DEFERRED | YES |
| 2 | Documents | MySQL schema for documents | Supabase only | New SQL file with `employee_documents` schema | New SQL file | Additive | None yet | None yet | Admin, HR | Table creation | Low | 🔵 DEFERRED | YES before implementation |
