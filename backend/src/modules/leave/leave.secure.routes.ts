...
leaveSecureRouter.get("/requests", h(async (req: any, res: any) => {
  ...
  if (req.query.activeOn) {
    conds.push("lr.from_date <= ?");
    conds.push("lr.to_date >= ?");
    params.push(String(req.query.activeOn), String(req.query.activeOn));
  }
  ...
  const [rows] = await db.execute<RowDataPacket[]>(
    `SELECT lr.*,
            ...
            lt.leave_code,
            COALESCE(NULLIF(TRIM(rev.full_name), ''), TRIM(CONCAT(rev.first_name, ' ', COALESCE(rev.last_name, '')))) AS reviewer_name,
            approval.action_at AS reviewed_at,
            approval.remarks AS review_notes
       ${fromSql}
       ${where}
...
leaveSecureRouter.patch("/requests/:id/review", h(async (req: any, res: any) => {
  if (!(await canReviewLeave(req.authUser!.id, req.params.id))) {
    return res.status(403).json({ success: false, message: "Forbidden: leave request is outside your approval scope" });
  }

  const status = String(req.body.status ?? "");
  const allowed = ["approved", "rejected", "branch_head_approved", "branch_head_rejected"];
  if (!allowed.includes(status)) {
    return res.status(400).json({ success: false, message: "Invalid leave review status" });
  }

  const data = await leaveService.reviewRequest(req.params.id, {
    status: status as any,
    remarks: req.body.remarks ?? req.body.reviewNotes ?? null,
  }, req.authUser!.id);
